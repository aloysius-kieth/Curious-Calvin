...
export default class PreloadScene extends BaseScene {
  ...
  preload() {
    ...
    this.loadingCircle.generateTexture('loadingcircle');
  ...
  loadAudio() {
    ...
    //#endregion
    this.load.audio(AudioConfig.MUSIC.IDLE_MUSIC.KEY, 'assets/audio/bgm/Cartoon_Orchestral_Background_1.mp3');
    ...
    this.load.audio(AudioConfig.SFX.GAIN_POINT.KEY, 'assets/audio/sfx/Bonus_Round_Game_Sound.mp3');
    this.load.audio(AudioConfig.SFX.POISON_HIT.KEY, 'assets/audio/sfx/Game_Poison_Dart_Spell.wav');
    this.load.audio(AudioConfig.SFX.GAME_OVER.KEY, 'assets/audio/sfx/gameover.mp3');
    this.load.audio(AudioConfig.SFX.DEATH.KEY, 'assets/audio/sfx/Cartoon_Character_Death_05.wav');
    this.load.audio(AudioConfig.SFX.BUTTON_CLICK.KEY, 'assets/audio/sfx/buttonclick.wav');
  }
  loadSprites() {
    this.load.image('startbutton', 'assets/img/game/startbutton.png');
  }
  loadPNGSeq() {
  }
  init() {
  ...
  create() {
    ...
    this.startButton = this.add.sprite(this.width, this.height, 'startbutton')
      .setScale(0, 0,)
      .setVisible(false);
    this.startText = this.add.text(this.startButton.x, this.startButton.y, 'Press\nto\nStart', {
      font: '32px Merck',
      align: 'center',
      color: '#FFC732'
    })
    ...
    this.delay = this.time.delayedCall(1000, () => {
      ...
      this.tweens.add({
        ...
        onComplete: () => {
          ...
          this.startButton.setInteractive().once('pointerdown', () => {
